...
### 4. 交换区服 (Swap Servers)
*   仅针对「正常组」。
*   在 XLSX 文件中定位 `S1` 和 `S2` 所在的行（`R1`, `R2`）。
*   **交换逻辑**：
    *   将 `R1` 行中的 `S1` 替换为 `S2`。
    *   将 `R2` 行中的 `S2` 替换为 `S1`。
    *   **排序修正**：交换后，重新检查这两行的配对，确保每一行的配对中，数值较小的 ID 作为【目标服】（在前），数值较大的作为【参与服】（在后）。
*   **样式标记**：将发生交换的行（`R1`, `R2`）背景标记为黄色。
*   **限制**：如果 `S1` 或 `S2` 在 XLSX 中找不到，或者它们恰好在同一行，则无法交换，并记录警告。
